...
export default class ThreeBall extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (companyList && companyList.length && !this.hasRender) {
      console.log('componentWillReceiveProps companyList', companyList)
  ...
  initScene = () => {
    ...
    this.setupScene();
    if (this.props.finishCB) { // 通知上层完成渲染
      this.props.finishCB(true)
      cookies.setCookie('finish_WEBGL', 1, '/', 10);
    }
  }
  ...
  renderSkyBox = () => {
    /*   const imagePrefix = './images/nebula-';
      const directions = ['xpos', 'xneg', 'ypos', 'yneg', 'zpos', 'zneg'];
      const imageSuffix = '.png'; */
    const imageURLs = [
      /* require('./images/nebula-xpos.png'),
      require('./images/nebula-xneg.png'),
      require('./images/nebula-ypos.png'),
      require('./images/nebula-yneg.png'),
      require('./images/nebula-zpos.png'),
      require('./images/nebula-zneg.png'), */
    ...
    /*    for (let i = 0; i < 6; i += 1) {
         const url = imagePrefix + directions[i] + imageSuffix;
         imageURLs.push(require(url))
       } */
    const materialArray = [];
  ...
  render() {
    ...
    return (
      showBloack ? (<div className={styles.my3DBalls}>
        <div className={classnames('loadWrapper', { hideWrapper: showCanvas })}>
          Loading.....
        </div>
        {info && <div className="infoModal">

          <p>公司名称:{info.name_zh}</p>
          <p>公司简介：{info.introduction_zh}</p>
        </div>}
        <div id="WebGL-output" ref={ref => this.myRef = ref}></div>
        <div className="fold" onClick={() => { this.setState({ showBloack: false }) }}>点击关闭</div>
      </div>) : null
    )
  }
}
