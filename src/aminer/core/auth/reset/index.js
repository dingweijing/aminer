  ...
  const handleSubmit = (e) => {
    ...
    validateFieldsAndScroll(['password', 'passwordComfirm'],(errors, values) => {
      ...
      dispatch({
        ...
        payload: { identifier: email, token, src, password }
      }).then((status) => {
        if (status) {
          history.push(`/login`)
          notification.success({
            message: formatMessage({ id: 'aminer.common.success', defaultMessage: 'Success!' }),
            description: formatMessage({ id: 'aminer.reset.success', defaultMessage: 'Password reset successfully' })
          });
        }
      }).catch(err => {
        notification.error({
      ...
      <article className={styles.article}>
        <section className={styles.content}>
          <div className={styles.loginHeader}>
            <div className={styles.registerTip1}><FM id='aminer.reset.tip1'/></div>
          </div>
