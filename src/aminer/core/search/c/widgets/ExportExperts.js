...
// TODO 导出字段移到sysconfig
const plainOptions = ['id', 'name', 'gender', 'pos', 'aff', 'h_index', 'activity', 'new_star', 'num_citation', 'num_pubs', 'interest'];

const defaultCheckedList = plainOptions; // ['name', 'pos', 'aff', 'h_index'];

@connect(({ app, exportExperts }) => ({ app, exportExperts, }))
@Auth
export default class ExportExperts extends Component {
  ...
  clickDownload = e => {
    ...
    const selected = plainOptions.filter(x => this.state.checkedList.includes(x));
    ...
    const fetchData = (data, size, offset, i) => {
      ...
      this.props.dispatch({
        ...
        payload: { query: typeof query === 'object' ? query.query : query, filters, sort, size: NewSize, offset, ghost: true },
      }).then(res => {
        // TODO res拼到data
