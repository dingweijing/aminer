...
const OrgPage = props => {
  ...
  const orgName = decodeURIComponent(org_name);
                    ...
                    return (
                      <li className="multiple_item fields_item" key={name}>
                        <span className="award_name">
                          {year && (
                            <Link to={`/${aiType}/year/${year}/${item.id}`} target="_black">
                              <strong>{name || name_zh || ''}</strong>
                            </Link>
                          )}
                          {!year && (
                            <Link to={`/${aiType}/${item.id}`} target="_black">
                              <strong>{name || name_zh || ''}</strong>
                            </Link>
                          )}
                        </span>
...
OrgPage.getInitialProps = async ({ store, history, match, isServer }) => {
  if (!isServer) {
    return;
  }
  ...
  const { pathname } = location;
  const { year, org_name } = match.params || {};
