...
// ------------------- 下面的都是老版本的service， 需要逐个替换掉。------------------------------
const api = {
  ...sapi,
  profileSimilar: `${baseURL}/person/similar/:id`,
  // personFollow: `${baseURL}/social/:id/follow`,
  profileDcore: `${baseURL}/person/indices/dcore/:id`,
  ProfileEgoNet: `${baseURL}/person/ego/:id?cache=true`,
  ProfileInfocard: `${baseURL}/person/infocard/:id`,
  ProfileRadarChart: `${baseURL}/person/indices/:id`,
  personRelModify: `${baseURL}/person/rel/modify`
};
...
export function getAMinerProfileUrl(name, id) {
  ...
  // return `/person/${str}/${id}`;
  return `${consts.AMinerURL}/profile/${str}/${id}`;
}

/** Visualization Data */
export async function getInterestVisDatass(personId) {
...
export function getInterestsI18N(callback) {
  console.warn('TODO do this in server!', 'getInterestsI18N')
  // // If cached.
...
export function returnKeyByLanguage(interestsData, key) {
  const tag = { en: key, zh: '' };
  if (interestsData && interestsData.data && sysconfig.Locale === zhCN) {
...
export async function getProfileSimilar(id) {
  return request(api.profileSimilar.replace(':id', id), {
    method: 'GET',
  });
}

export async function followPerson(id) {
  const params = { pid: id };
  return request(api.personFollow.replace(':id', id), {
    method: 'POST',
    body: JSON.stringify(params),
  });
}

export async function unFollowPerson(id) {
  const params = { pid: id };
  return request(api.personFollow.replace(':id', id), {
    method: 'DELETE',
    body: JSON.stringify(params),
...
export async function getProfileDcore(params) {
  const nextapi = apiBuilder.create(Action.personapi.GetDCoreIndicesByAid, 'GetDCoreIndicesByAid')
  .param(params);
  return nextAPI({ data: [nextapi.api], type: 'n' });
}
// export async function getProfileDcore(id) {
//   return request(api.profileDcore.replace(':id', id), {
//     method: 'GET',
//   });
// }

export async function getProfileEgoNet(params) {
  const nextapi = apiBuilder.create(Action.personapi.GetEgoNetworkGraph, 'GetEgoNetworkGraph')
  .param(params);
  return nextAPI({ data: [nextapi.api], type: 'n' });
}
// export async function getProfileEgoNet(id) {
//   return request(api.ProfileEgoNet.replace(':id', id), {
//     method: 'GET',
//   });
// }

export async function getProfileInfocard(id) {
  return request(api.ProfileInfocard.replace(':id', id), {
    method: 'GET',
  });
}

export async function getProfileRadarChart(id) {
  console.log(api.ProfileRadarChart);
  return request(api.ProfileRadarChart.replace(':id', id), {
    method: 'GET',
  });
}

export async function setPersonRelModify(params) {
  return request(api.personRelModify, {
    method: 'POST',
    body: JSON.stringify(params),
  });
}
